...
public class SimpleJavaClientGenerator extends AbstractJavaClientGenerator {
    ...
    public List<CompilationUnit> getCompilationUnits() {
        ...
        // DAO UNIT
        // ALTER BY STRIVEYEE MAPPER TO DAO
        FullyQualifiedJavaType type = new FullyQualifiedJavaType(
                introspectedTable.getDAOInterfaceType());//.getMyBatis3JavaMapperType());
...
//        addUpdateByPrimaryKeyMethod(interfaze);
        ...
        // ALTER BY STRIVEYEE : ADD DAO IMPL
        // DAO IMPL UNIT
        FullyQualifiedJavaType implType = new FullyQualifiedJavaType(
                introspectedTable.getDAOImplementationType().replace("dao", "dao.impl"));//.getMyBatis3JavaMapperType());
        TopLevelClass implClass = new TopLevelClass(implType);
        interfaze.setVisibility(JavaVisibility.PUBLIC);

        // impl dao
        FullyQualifiedJavaType inf_fqjt = new FullyQualifiedJavaType(
                interfaze.getType().getShortName());
        implClass.addSuperInterface(inf_fqjt);
        interfaze.addImportedType(inf_fqjt);

        if (context.getPlugins().clientGenerated(null, implClass,
                introspectedTable)) {
            answer.add(implClass);
        }
